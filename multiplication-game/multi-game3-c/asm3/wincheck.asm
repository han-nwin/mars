...
# has_valid_moves: Checks if any product of top and bottom markers (1-9) corresponds to an unclaimed cell
# Output: $v0 = 1 if at least one valid move exists, 0 if no valid moves
has_valid_moves:
    addi $sp, $sp, -20        # Allocate 20 bytes for 5 registers
    sw $ra, 0($sp)            # Save return address
    sw $s0, 4($sp)            # Save $s0 (top marker counter)
    sw $s1, 8($sp)            # Save $s1 (bottom marker counter)
    sw $s2, 12($sp)           # Save $s2 (product)
    sw $s3, 16($sp)           # Save $s3 (row from find_cell)

    li $s0, 1                 # Top marker counter (1-9)
top_marker_loop:
    li $s1, 1                 # Bottom marker counter (1-9)
bottom_marker_loop:
    mul $s2, $s0, $s1         # Calculate product (top * bottom)
    move $a0, $s2             # Set product as argument
    jal find_cell             # Find cell for this product
    move $s3, $v0             # Save row (from $v0)
    move $t0, $v1             # Save column (from $v1)
    li $t1, -1                # Check if cell is invalid
    beq $s3, $t1, next_bottom # If row = -1, product not in grid, try next
    move $a0, $s3             # Set row for get_ownership
    move $a1, $t0             # Set column
    jal get_ownership         # Check if cell is unclaimed
    beqz $v0, found_valid_move # If ownership = 0, cell is unclaimed, valid move found
next_bottom:
    addi $s1, $s1, 1          # Increment bottom marker
    li $t0, 10                # Upper bound (1-9, so < 10)
    blt $s1, $t0, bottom_marker_loop # If bottom < 10, try next bottom marker
    addi $s0, $s0, 1          # Increment top marker
    li $t0, 10                # Upper bound
    blt $s0, $t0, top_marker_loop # If top < 10, try next top marker

    li $v0, 0                 # No valid moves found
    j valid_moves_cleanup
found_valid_move:
    li $v0, 1                 # Valid move found
valid_moves_cleanup:
    lw $ra, 0($sp)            # Restore return address
    lw $s0, 4($sp)            # Restore $s0
    lw $s1, 8($sp)            # Restore $s1
